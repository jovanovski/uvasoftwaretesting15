...
-- what i found during testing fermat's algorithm with carmichael numbers is that suprisingly for some of the 
-- numbers in the carmichael list the algorithm returns false meaning that it classifies the number as a composite.
-- but we know that carmichael numbers are composite numbers that have the property of: a^(n-1) = 1 (mod n) so i expected
...
-- above, the first few numbers in the list are skiped. but i finaly realized what is the reason behind this. as stated in the
-- wikipedia page for carmicheal numbers: a carmicheal number is a composite number n which satisfies b^(n-1) = 1 (mod n) for all
-- Integers 1<a<n which are relatively prime to n. at first i did not pay attention to the second part of the property. the second part
